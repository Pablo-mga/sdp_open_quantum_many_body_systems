...
def non_translation_invariant_sdp(k,L_op,local_obs_array):
    ...
    for i in range(num_rho-1):
        h_0=build_hamiltonian(2, J, g,i)
        h_dif=build_hamiltonian(1, J, g,i+1)
        h_dif=kron(id2,h_dif)
        h_0=h_0-h_dif
        for i1 in range(k-2):
            h_0=kron(h_0,id2)
        h_1=build_hamiltonian(k, J, g,i+1)
...
results_dir = "results_non_ti_homogeneous_field"
...
for obs_label, obs in observables.items():
    ...
    # Create the list of local observables
    local_obs_array = []

    for site in range(L // 2):
        local_obs_array.append({
            "ops": [obs],
            "sites": [site],
        })

    # Compute and save SDP limits
