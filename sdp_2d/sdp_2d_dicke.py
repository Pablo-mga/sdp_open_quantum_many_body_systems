...
def initialize_problem(k):
    ...
    constraints_s = [rho >> 0,cp.trace(rho) == 1]
    ...
    mat_d = sp.sparse.load_npz(os.path.join(folder_name, 'diss_flattened_dicke.npz'))
    
    lindblad_vector = (g_param * mat_x  + gamma * mat_d)
...
folder = "results_dicke"
os.makedirs(folder, exist_ok=True)

df.to_csv(f"{folder}/observable_bounds{system}".replace('.', '_') + ".csv",index=False)
